...
yetki_kontrol(['personel', 'admin']);

// Personel bilgilerini al
$personel_id = $_SESSION['kullanici']['kullanici_id'];
$personel = $db->prepare("CALL kullanici_adsoyad_al(?)");
$personel->execute([$personel_id]);
$personel = $personel->fetch(PDO::FETCH_ASSOC);

// Randevu durum güncelleme
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['randevu_durum'])) {
        try {
            $stmt = $db->prepare("CALL randevu_durum_guncelle(?, ?, @sonuc)");
            $stmt->execute([$_POST['randevu_id'], $_POST['durum_id']]);
            
            $_SESSION['mesaj'] = "Randevu durumu güncellendi!";
    ...
    if (isset($_POST['saat_kapat'])) {
        try {
            ...
            $_SESSION['mesaj'] = "Kapalı saat eklendi!";
        } catch (PDOException $e) {
            $_SESSION['hata'] = "Hata: " . $e->getMessage();
        }
    }
    ...
    // Kapalı saat silme
    if (isset($_POST['saat_sil'])) {
        try {
            $stmt = $db->prepare("CALL kapali_saat_sil(?, ?, @sonuc)");
            $stmt->execute([$_POST['saat_id'], $personel_id]);
    ...
    if (isset($_POST['saat_guncelle'])) {
        try {
            // Tarih formatını dönüştür (d.m.Y -> Y-m-d)
            $tarih_obj = DateTime::createFromFormat('d/m/Y', $_POST['guncelle_tarih']);
            if (!$tarih_obj) {
                throw new Exception("Geçersiz tarih formatı!");
            }
            $tarih = $tarih_obj->format('Y-m-d');
...
// Personelin randevularını getir (bugün ve sonrası)
$randevular = $db->prepare("CALL personel_randevulari(?)");
$randevular->execute([$personel_id]);
$randevular = $randevular->fetchAll();

// Kapalı saatleri getir
$kapali_saatler = $db->prepare("CALL kapali_saat_listesi(?)");
$kapali_saatler->execute([$personel_id]);
$kapali_saatler = $kapali_saatler->fetchAll();
...
<head>
    ...
    <title>Personel Paneli - <?php echo htmlspecialchars($personel['ad']." ".$personel['soyad']); ?></title>
